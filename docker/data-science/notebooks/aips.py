...
def render_search_results(query, results):
    ...
    with open(search_results_template_file) as file:
        ...
        for result in results:
            rendered += results_template.replace("${NAME}", result['name'][0] if 'name' in result else "UNKNOWN") \
                .replace("${MANUFACTURER}", result['manufacturer'][0] if 'manufacturer' in result else "UNKNOWN") \
                .replace("${DESCRIPTION}", result['shortDescription'][0] if 'shortDescription' in result else "") \
                .replace("${IMAGE_URL}", "../data/retrotech/images/" + (result['upc'] if 'upc' in result else "UNKNOWN") + ".jpg")
